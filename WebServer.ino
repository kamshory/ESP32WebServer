...
void saveSubData() {
  if(server.method() == HTTP_POST)
  {
    ...
    String savedMQTTPassowrd = server.arg("mqtt_password");
    writeData(offsetMQTTHost, eepromDataLength, savedMQTTPassowrd);
  }
  String message = "{\"status\":\"OK\"}";
  server.send(200, "application/json", message); 
}
...
void setup(void) {
  ...
  String ssidStaS = readDataString(offsetSSID1, eepromDataLength);
  String passwordStaS = readDataString(offsetSSIDPassword1, eepromDataLength);
